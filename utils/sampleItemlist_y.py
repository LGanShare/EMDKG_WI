...
def gen_itemlist_random_y(candid_items,ui_weights,item_features,features=None,div_type='cc',delta=3,num_features=18,max_per_user=50000):
    """
    ...
    for u in candid_items:
        ...
        if not features is None:
            for e in item_features:
                if not set(item_features[e]).issubset(set(features)):
                    cple_items.append(e)
        else:
            cple_items = [e for e in item_features if e in items]
        if len(cple_items) == 0:
            cple_items = [e for e in item_features if e in items]
        j = 0
        ...
        _val_u = []
        count_loop = 0
        while len(res_u) < max(max_per_user,60000) and count_loop < 60000:
            count_loop += 1
            if count_loop%2==1:
                # len(items) >= 5:
                candid_list_u = set(random.sample(items,8) + random.sample(cple_items,2))
            else:
                if len(items) >= 10:
