...
class SimuConfigX(object):
    ...
    def train_one_step(self):
        ...
        loss = self.trainRecomModel() + F.kl_div(self.trainRecomModel.item_embeddings.data, self.trainDivItemModel.item_embedding.data)
        loss.backward()
    ...
    def train(self, train_data, valid_data=None,prop=3):
        ...
        for epoch in range(self.train_times):
            ...
            if (epoch + 1) % self.valid_steps == 0:
                ...
                if (theta*hit_recom+(1-theta)*hit_DivItem) > best_hit:
                    best_hit = theta*hit_recom + (1-theta)*hit_DivItem
                    best_epoch = epoch
